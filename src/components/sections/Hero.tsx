    ...
    >
      ...
      <div className="relative z-20 w-full container mx-auto px-6 sm:px-8 py-28 flex flex-col lg:flex-row items-center justify-center gap-10 lg:gap-20 min-h-screen">
        ...
        >
          ...
          <h1 className="font-hero text-[3.5rem] sm:text-7xl lg:text-8xl text-white drop-shadow-2xl leading-tight mb-7">
              ...
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2 z-20">
                {slides.map((_: string, i: number) => (
                  <button key={i} onClick={() => setCurrentSlide(i)}
                    className={`rounded-full transition-all duration-300 ${i === currentSlide ? 'w-5 h-1.5 bg-white' : 'w-1.5 h-1.5 bg-white/50'}`}
                  />
                ))}
              </div>
            )}
